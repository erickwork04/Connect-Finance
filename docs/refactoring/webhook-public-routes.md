...
> Atualização local após a Etapa 7: com Next.js 15.5.24 e Clerk 6.39.7, `GET` nas duas rotas de webhook retornou 405 sem redirect; `/dashboard` sem sessão continuou em 307 para `/login`. O restante abaixo registra a implementação original das Etapas 2/3. Callback real continua **PENDENTE — validação com credenciais/ambiente real do Mercado Pago**.
