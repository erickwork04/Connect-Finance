...
export default function CategoryExpensesCard({ data, period }: { data: DashboardData; period: CategoryPeriod }) {
  let cursor = 0;
  const slices = data.categories.map((item) => {
    const from = cursor;
    cursor += item.percent;
  ...
  return <section className="min-w-0 rounded-2xl border border-white/10 bg-[#141816] p-4 sm:p-5 lg:p-5">
    ...
    <div className="mt-5 grid items-center gap-6 sm:grid-cols-[minmax(0,220px)_1fr] lg:mt-4 lg:max-w-[570px] lg:grid-cols-[180px_minmax(0,1fr)] lg:gap-4">
